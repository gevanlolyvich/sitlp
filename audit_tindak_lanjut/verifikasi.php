...
<tr><th width="220">Nomor TL</th><td><?= htmlspecialchars($tl['nomor_tindak_lanjut']) ?></td></tr>
 ...
 <div class="d-flex mb-2">
  ...
  <div>
   ...
   <?php if ($log['aksi'] == 'verifikasi'): ?>
    <div>
     <?php if ($log['status_lama']): ?><span class="badge bg-secondary"><?= htmlspecialchars($log['status_lama']) ?></span> <i class="fas fa-arrow-right mx-1"></i><?php endif; ?>
   ...
   <?php elseif ($log['aksi'] == 'upload_bukti'): ?>
    ...
    <?php if ($log['file_bukti']): ?><a href="../uploads/tindak_lanjut/<?= $log['file_bukti'] ?>" target="_blank" class="btn btn-xs btn-success mt-1">Lihat Bukti</a><?php endif; ?>
   <?php endif; ?>
  </div>
 </div>
 <?php if ($lidx < count($logs) - 1): ?><div class="text-center text-muted" style="margin-left:20px;">↓</div><?php endif; ?>
 <?php endforeach; ?>
</div>
</div>
<?php endif; ?>
...
* <strong>Sesuai</strong> = Final, tidak bisa diubah lagi, Auditee tidak bisa upload ulang.<br>
* <strong>Belum Sesuai / Belum Ditindak Lanjut / Tidak Dapat Ditindak Lanjut</strong> = Perlu revisi, Auditee bisa upload ulang.
</div>
</div>

<div class="mb-3">
<label>Catatan SPI</label>
<textarea name="catatan_spi" class="form-control" rows="4" placeholder="Catatan untuk auditee..."><?= htmlspecialchars($tl['catatan_spi'] ?? '') ?></textarea>
</div>
